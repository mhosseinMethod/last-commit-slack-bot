...
function formatRepoHistory(data) {
  const MAX_CHARS = 2800; // safety margin under Slack's 3000 limit
  ...
  // Optional repo header if we found it
  if (repo) {
    // lines.push(`*📦 Repo:* <${repoUrl}|${repo}>`, "");
  }

  // AI Summary block (from data.summary)
  //   if (data.summary) {
  //     lines.push(`*AI Summary:* ${data.summary}`, "");
  //   } else {
  //     lines.push(`*AI Summary:* (no summary available)`, "");
  //   }
